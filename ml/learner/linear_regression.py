...
class LinRegOHLCV:
    ...
    def build_model(self, fit_intercept=True):
...
def run_trial(trial_number, df, aggregated_df):
    fit_intercept = bool(np.random.choice([True, False]))
    lookahead = int(np.random.choice(range(1, 4)))
    threshold = float(round(np.random.uniform(0.001, 0.01), 4))

    print(f"\n=== Trial {trial_number} ===")
    print(f"Params: fit_intercept={fit_intercept}, lookahead={lookahead}, threshold={threshold}")

    df_labeled = generate_labels(aggregated_df, lookahead, threshold)

    linreg = LinRegOHLCV(df_labeled)
    linreg.preprocess_data()
    linreg.build_model(fit_intercept=fit_intercept)
    linreg.train()

    signals_df = linreg.predict().dropna()
        ...
        for sym in symbol_list:
            for time in time_horizon_list:
                ...
                for rank, trial in enumerate(top_trials, start=1):
                    ...
                    table_name = f"{exch}_{sym}_{time}_linear_regression_trial{trial['trial']}".lower()
                    store_signal_stegnth_data(schema_name, table_name, trial["pnl_df"])

                print("\n✅ Top Linear Regression trials saved with pickle weights, metadata, and DB records!")
