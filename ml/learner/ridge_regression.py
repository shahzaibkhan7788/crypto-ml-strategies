...
class RidgeOHLCV:
    ...
    def build_model(self, alpha=1.0, solver='auto', max_iter=1000):
        self.model = Ridge(
            alpha=alpha,
            solver=solver,
            max_iter=max_iter
...
def run_trial(trial_number, df, aggregated_df):
    alpha = float(round(np.random.uniform(0.001, 10.0), 3))
    solver = np.random.choice(['auto', 'svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga'])
    max_iter = int(np.random.choice([500, 1000, 1500]))
    lookahead = int(np.random.choice(range(1, 4)))
    threshold = float(round(np.random.uniform(0.001, 0.01), 4))

    print(f"\n=== Trial {trial_number} ===")
    print(f"Params: alpha={alpha}, solver={solver}, max_iter={max_iter}, lookahead={lookahead}, threshold={threshold}")

    df_labeled = generate_labels(aggregated_df, lookahead, threshold)

    model = RidgeOHLCV(df_labeled)
    model.preprocess_data()
    model.build_model(alpha=alpha, solver=solver, max_iter=max_iter)
    model.train()
        ...
        for sym in symbol_list:
            for time in time_horizon_list:
                ...
                for rank, trial in enumerate(top_trials, start=1):
                    ...
                    table_name = f"{exch}_{sym}_{time}_ridge_regression_trial{trial['trial']}".lower()
                    store_signal_stegnth_data(schema_name, table_name, trial["pnl_df"])

                print("\n✅ Top Ridge Regression trials saved with pickle weights, metadata, and DB records!")
