...
class XGBRegOHLCV:
    ...
    def preprocess_data(self):
        ...
        y = self.df['signal'].values  # using generated signals
        self.X = self.scaler.fit_transform(X)
        self.y = y

    def build_model(self, n_estimators=100, max_depth=3, learning_rate=0.1, subsample=1.0, colsample_bytree=1.0):
...
def run_trial(trial_number, df, aggregated_df):
    n_estimators = int(np.random.choice(range(50, 301, 50)))
    ...
    colsample_bytree = float(round(np.random.uniform(0.5, 1.0), 2))
    ...
    xgb_model = XGBRegOHLCV(df_labeled)
    xgb_model.preprocess_data()
    ...
    signals_df = xgb_model.predict().dropna()
        ...
        for sym in symbol_list:
            for time in time_horizon_list:
                ...
                for rank, trial in enumerate(top_trials, start=1):
                    ...
                    table_name = f"{exch}_{sym}_{time}_xg_boost_trial{trial['trial']}".lower()
                    store_signal_stegnth_data(schema_name, table_name, trial["pnl_df"])

                print("\n✅ Top XGBoost Regression trials saved with pickle weights, metadata, and DB records!")
