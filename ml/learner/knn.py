...
class KNNRegOHLCV:
    ...
    def build_model(self, n_neighbors=5, weights='uniform', p=2):
...
def run_trial(trial_number, df, aggregated_df):
    n_neighbors = int(np.random.choice(range(3, 21)))
    weights = np.random.choice(['uniform', 'distance'])
    p = int(np.random.choice([1, 2]))  # 1=Manhattan, 2=Euclidean
    lookahead = int(np.random.choice(range(1, 4)))
    threshold = float(round(np.random.uniform(0.001, 0.01), 4))

    print(f"\n=== Trial {trial_number} ===")
    print(f"Params: n_neighbors={n_neighbors}, weights={weights}, p={p}, "
          f"lookahead={lookahead}, threshold={threshold}")

    df_labeled = generate_labels(aggregated_df, lookahead, threshold)

    knn_model = KNNRegOHLCV(df_labeled)
    knn_model.preprocess_data()
    knn_model.build_model(n_neighbors=n_neighbors, weights=weights, p=p)
    knn_model.train()

    signals_df = knn_model.predict().dropna()
            ...
            for time in time_horizon_list:
                ...
                for rank, trial in enumerate(top_trials, start=1):
                    metadata = {
                        ...
                        'symbol': sym,
                        'time_horizon': time,
                        'model_name': 'knn_regression_model',
                    ...
                    table_name = f"{exch}_{sym}_{time}_knn_trial{trial['trial']}".lower()
                    store_signal_stegnth_data(schema_name, table_name, trial["pnl_df"])

                print("\n✅ Top KNN Regression trials saved with pickle weights, metadata, and DB records!")
