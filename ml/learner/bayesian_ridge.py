...
# =========================
# Bayesian Ridge Regression Model Class
# =========================
class BayesianRidgeOHLCV:
    ...
    def build_model(self, n_iter=300, alpha_1=1e-6, alpha_2=1e-6, lambda_1=1e-6, lambda_2=1e-6):
...
def run_trial(trial_number, df, aggregated_df):
    # Randomly choose Bayesian Ridge hyperparameters
    n_iter = int(np.random.choice(range(100, 1001, 100)))
    alpha_1 = float(round(np.random.uniform(1e-7, 1e-4), 7))
    alpha_2 = float(round(np.random.uniform(1e-7, 1e-4), 7))
    lambda_1 = float(round(np.random.uniform(1e-7, 1e-4), 7))
    lambda_2 = float(round(np.random.uniform(1e-7, 1e-4), 7))
    lookahead = int(np.random.choice(range(1, 4)))
    threshold = float(round(np.random.uniform(0.001, 0.01), 4))

    print(f"\n=== Trial {trial_number} ===")
    print(f"Params: n_iter={n_iter}, alpha_1={alpha_1}, alpha_2={alpha_2}, "
          f"lambda_1={lambda_1}, lambda_2={lambda_2}, lookahead={lookahead}, threshold={threshold}")

    df_labeled = generate_labels(aggregated_df, lookahead, threshold)

    model = BayesianRidgeOHLCV(df_labeled)
    model.preprocess_data()
    model.build_model(n_iter=n_iter, alpha_1=alpha_1, alpha_2=alpha_2, lambda_1=lambda_1, lambda_2=lambda_2)
    model.train()
            ...
            for time in time_horizon_list:
                ...
                for rank, trial in enumerate(top_trials, start=1):
                    metadata = {
                        ...
                        'symbol': sym,
                        'time_horizon': time,
                        'model_name': 'bayesian_ridge_model',
                    ...
                    table_name = f"{exch}_{sym}_{time}_bayesian_ridge_trial{trial['trial']}".lower()
                    store_signal_stegnth_data(schema_name, table_name, trial["pnl_df"])

                print("\n✅ Top Bayesian Ridge Regression trials saved with pickle weights, metadata, and DB records!")
